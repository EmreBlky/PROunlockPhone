$(function() {
    ...
    $("#imei").on("keyup keypress blur change", function(){
        var $imei = $(this).val();
        if(!isNaN($imei) && $imei.length == 14) {
            ...
            $.get('Check_IMEI.php?imei=' + $imei + last_digit + '&service=' + $("#service").val(), function( response ) {
                if(response != "0") {
                    ...
                    updateTips("Notification: This IMEI was previously ordered with this same service.<br />Note that you still can place your order, nevertheless make sure you are not double ordering it and ensure that you solved any previous issue that might have caused rejection. Contact the administrator if you have any question <a href='mailto:support@prounlockphone.com'>support@prounlockphone.com</a>");
                }// else {
                    $("#last_digit").val(last_digit);
                    $("#imei").css("color", "green");
                    $("#imei").css("border", "solid 1px gray");
                //}
            });
        } else {
            $("#last_digit").val("");
