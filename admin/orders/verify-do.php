...
if($data['status'] == "Success" and $data['checkRequest'] == 0 and $data['checkRevoke'] == 0 and (strtotime(date("Y-m-d H:i")) - strtotime($data['last_update']) < 432000)) {
    ...
    $body = "Client: {$data['first_name']} {$data['last_name']} ({$data['username']}: {$data['email']})<br />"
        ...
        . "<br />"
        . "Reason:";
    if ($_POST['completed']) $body .= "<br />- Order is not completed";
    if ($_POST['expected']) $body .= "<br />- Results do not match what you expected";
    if ($_POST['unsatisfied']) $body .= "<br />- Protest results, unsatisfied";
    if ($_POST['submit']) $body .= "<br />- You did not submit this order";
    if ($_POST['reprocess']) $body .= "<br />- Code / Results can't be read, need to reprocess";
