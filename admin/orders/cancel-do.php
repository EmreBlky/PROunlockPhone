...
if(($data['status'] == "Pending" or $data['status'] == "In process") and $data['cancelRequest'] == 0 and $data['cancelRevoke'] == 0 and $data['checkRequest'] == 0) {
    ...
    " . $row["admin_response_comments"] : "";
        ...
        $body = "Client: {$data['first_name']} {$data['last_name']} ({$data['username']})<br />"
            ...
            . "<br />"
            . "Reason:";
            if($_POST['time']) $body .= "<br />- Order passed the posted processing time";
            if($_POST['stop']) $body .= "<br />- You need to stop at any cost, maybe due to your client or another reason";
            if($_POST['wrong']) $body .= "<br />- Wrong service, willing to order the appropriate one";
            if($_POST['completed']) $body .= "<br />- The order is completed and no need to the service";
            if($_POST['cheaper']) $body .= "<br />- You found cheaper service";
            if($_POST['other'] != "") $body .= "<br />- Other: " . nl2br($_POST['other']);
        require_once('../eMail.php');
